...
pub fn build() -> RuntimeValue {
    ...
    map.insert(
        ...
        make_builtin("string.join", |args, l, c| {
            ...
            Err(CompilerError::TypeMismatch {
                operation: "string.join".into(),
                expected: "Array and String".into(),
                found: format!("{}, {}", args[0].type_name(), args[1].type_name()),
                line: l,
                column: c,
            })
    ...
    map.insert(
        ...
        make_builtin("string.replace", |args, l, c| {
            ...
            if let (RuntimeValue::String(s), RuntimeValue::String(old), RuntimeValue::String(new)) =
                (&args[0], &args[1], &args[2])
            {
                return Ok(RuntimeValue::String(s.replace(old, new)));
            }
            Err(CompilerError::TypeMismatch {
                operation: "string.replace".into(),
        ...
        make_builtin("string.substring", |args, l, c| {
            ...
            ) = (&args[0], &args[1], &args[2])
            {
                ...
                return Err(CompilerError::RuntimeException {
                    message: format!(
                        "index out of bounds for substring: start={}, end={}, len={}",
                        start,
                        end,
                        s.len()
                    ),
            ...
            Err(CompilerError::TypeMismatch {
                operation: "string.substring".into(),
                expected: "String, Integer, and Integer".into(),
                found: format!(
                    "{}, {}, {}",
                    args[0].type_name(),
                    args[1].type_name(),
