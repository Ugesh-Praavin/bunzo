...
pub fn build() -> RuntimeValue {
        ...
        make_builtin("networking.tcp_listen", |args, l, c| {
            if args.len() != 1 {
                return Err(CompilerError::ArityMismatch {
                    name: "networking.tcp_listen".into(),
                    expected: 1,
                    found: args.len(),
                    line: l,
                    column: c,
            ...
            Err(CompilerError::TypeMismatch {
                operation: "networking.tcp_listen".into(),
    ...
    map.insert(
        ...
        make_builtin("networking.tcp_accept", |args, l, c| {
            if args.len() != 1 {
                return Err(CompilerError::ArityMismatch {
                    name: "networking.tcp_accept".into(),
                    expected: 1,
                    found: args.len(),
                    line: l,
                    column: c,
            ...
            if let RuntimeValue::Integer(id) = &args[0] {
                ...
                return Err(CompilerError::RuntimeException {
                    message: format!("invalid TCP listener handle: {id}"),
                    line: l,
                    column: c,
                });
            }
            Err(CompilerError::TypeMismatch {
                operation: "networking.tcp_accept".into(),
    ...
    map.insert(
        ...
        make_builtin("networking.tcp_connect", |args, l, c| {
            if args.len() != 1 {
                return Err(CompilerError::ArityMismatch {
                    name: "networking.tcp_connect".into(),
                    expected: 1,
                    found: args.len(),
                    line: l,
                    column: c,
            ...
            if let RuntimeValue::String(addr) = &args[0] {
                match TcpStream::connect(addr) {
                    ...
                    Err(e) => {
                        return Err(CompilerError::RuntimeException {
                            message: format!("failed to connect TCP to {addr}: {e}"),
                            line: l,
                            column: c,
                        });
                    }
            ...
            Err(CompilerError::TypeMismatch {
                operation: "networking.tcp_connect".into(),
                expected: "String".into(),
                found: args[0].type_name().to_string(),
                line: l,
                column: c,
            ...
            if let (RuntimeValue::Integer(id), RuntimeValue::String(data)) = (&args[0], &args[1]) {
                ...
                if let Some(mut stream) = stream {
                    match stream.write_all(data.as_bytes()) {
                        ...
                        Err(e) => {
                            return Err(CompilerError::RuntimeException {
                                message: format!("failed to send TCP data: {e}"),
                                line: l,
                                column: c,
                            });
                        }
                ...
                return Err(CompilerError::RuntimeException {
                    message: format!("invalid TCP stream handle: {id}"),
                    line: l,
                    column: c,
                });
            }
            Err(CompilerError::TypeMismatch {
                operation: "networking.tcp_send".into(),
                expected: "Integer and String".into(),
    ...
    map.insert(
        ...
        make_builtin("networking.tcp_recv", |args, l, c| {
            ...
            if let RuntimeValue::Integer(id) = &args[0] {
                ...
                if let Some(mut stream) = stream {
                    ...
                    match stream.read(&mut buf) {
                        ...
                        Err(e) => {
                            return Err(CompilerError::RuntimeException {
                                message: format!("failed to receive TCP data: {e}"),
                                line: l,
                                column: c,
                            });
                        }
            ...
            Err(CompilerError::TypeMismatch {
                operation: "networking.tcp_recv".into(),
                expected: "Integer".into(),
                found: args[0].type_name().to_string(),
                line: l,
                column: c,
    ...
    map.insert(
        ...
        make_builtin("networking.udp_bind", |args, l, c| {
            ...
            Err(CompilerError::TypeMismatch {
                operation: "networking.udp_bind".into(),
                expected: "String".into(),
                found: args[0].type_name().to_string(),
                line: l,
                column: c,
            ...
            ) = (&args[0], &args[1], &args[2])
            {
                ...
                if let Some(sock) = sock {
                    match sock.send_to(data.as_bytes(), addr) {
                        ...
                        Err(e) => {
                            return Err(CompilerError::RuntimeException {
                                message: format!("failed to send UDP packet to {addr}: {e}"),
                                line: l,
                                column: c,
                            });
                        }
                ...
                return Err(CompilerError::RuntimeException {
                    message: format!("invalid UDP socket handle: {id}"),
                    line: l,
                    column: c,
                });
            }
            Err(CompilerError::TypeMismatch {
                operation: "networking.udp_send".into(),
                expected: "Integer, String, and String".into(),
                found: format!(
                    "{}, {}, {}",
                    args[0].type_name(),
                    args[1].type_name(),
    ...
    map.insert(
        ...
        make_builtin("networking.udp_recv", |args, l, c| {
            ...
            if let RuntimeValue::Integer(id) = &args[0] {
                ...
                if let Some(sock) = sock {
                    ...
                    match sock.recv_from(&mut buf) {
                        ...
                        Err(e) => {
                            return Err(CompilerError::RuntimeException {
                                message: format!("failed to receive UDP packet: {e}"),
                                line: l,
                                column: c,
                            });
                        }
            ...
            Err(CompilerError::TypeMismatch {
                operation: "networking.udp_recv".into(),
                expected: "Integer".into(),
                found: args[0].type_name().to_string(),
                line: l,
                column: c,
