...
use bzc::ast::{BinaryOperator, Expression, Program, Statement, UnaryOperator};
use bzc::diagnostics::CompilerError;
use bzc::runtime::environment::Environment;
use bzc::runtime::eval::Interpreter;
use bzc::runtime::value::RuntimeValue;
...
fn test_literal_eval() {
    assert_eq!(
        eval_expr(Expression::IntegerLiteral {
            value: 42,
            line: 1,
            column: 1
        })
    ...
    assert_eq!(
        eval_expr(Expression::FloatLiteral {
            value: 3.14,
            line: 1,
            column: 1
        })
    ...
    assert_eq!(
        eval_expr(Expression::StringLiteral {
    ...
    assert_eq!(
        eval_expr(Expression::BooleanLiteral {
            value: true,
            line: 1,
            column: 1
        })
...
fn test_unary_negate() {
    assert_eq!(
        eval_expr(Expression::UnaryOp {
            ...
            operand: Box::new(Expression::IntegerLiteral {
                value: 5,
                line: 1,
                column: 1
            }),
    ...
    assert_eq!(
        eval_expr(Expression::UnaryOp {
            operator: UnaryOperator::Negate,
            operand: Box::new(Expression::FloatLiteral {
                value: 1.5,
                line: 1,
                column: 1
            }),
    ...
    let err = eval_expr(Expression::UnaryOp {
        operator: UnaryOperator::Negate,
        operand: Box::new(Expression::StringLiteral {
            value: "x".to_string(),
            line: 1,
            column: 1,
        }),
...
fn test_unary_not() {
    assert_eq!(
        eval_expr(Expression::UnaryOp {
            ...
            operand: Box::new(Expression::BooleanLiteral {
                value: false,
                line: 1,
                column: 1
            }),
...
fn test_division_by_zero() {
    ...
    assert!(matches!(
        err_float,
        Err(CompilerError::DivisionByZero { .. })
    ));
}
...
fn test_duplicate_declaration() {
    ...
    assert!(matches!(
        err,
        Err(CompilerError::DuplicateDeclaration { .. })
    ));
}
...
fn test_const_reassignment_in_environment() {
    ...
    env.define("PI".to_string(), RuntimeValue::Float(3.14), true, 1, 1)
        .unwrap();
